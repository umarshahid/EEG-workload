...
for w=1:1:L
    ...
    if (w >= 1 && w <= 4*20)        % Delta frequency range (1-4 Hz), signal length (20 sec)
        Delta_Frequencies(w) = 1;   % 1-4 hz frequencies are on
        Theta_Frequencies(w) = 0;
        Alpha_Frequencies(w) = 0;
        Beta_Frequencies(w) = 0;
        Gamma_Frequencies(w) = 0;
    elseif (w >= 4*20 && w <= 8*20)     % Theta frequency range (4-8 Hz), signal length (20 sec)
        Delta_Frequencies(w) = 0;
        Theta_Frequencies(w) = 1;       % 4-8 hz frequencies are on
        Alpha_Frequencies(w) = 0;
        Beta_Frequencies(w) = 0;
        Gamma_Frequencies(w) = 0;
    elseif (w >= 8*20 && w <= 13*20)    % Alpha frequency range (8-13 Hz), signal length (20 sec)
        Delta_Frequencies(w) = 0;
        Theta_Frequencies(w) = 0;
        Alpha_Frequencies(w) = 1;       % 8-13 hz frequencies are on
        Beta_Frequencies(w) = 0;
        Gamma_Frequencies(w) = 0;
    elseif (w >= 13*20 && w <= 30*20)   % Beta frequency range (13-30 Hz), signal length (20 sec)
        Delta_Frequencies(w) = 0;
        Theta_Frequencies(w) = 0;
        Alpha_Frequencies(w) = 0;
        Beta_Frequencies(w) = 1;        % 13-30 hz frequencies are on
        Gamma_Frequencies(w) = 0;
    elseif (w >= 30*20 && w <= 45*20)   % Gamma frequency range (30-45 Hz), signal length (20 sec)
        Delta_Frequencies(w) = 0;
        Theta_Frequencies(w) = 0;
        Alpha_Frequencies(w) = 0;
        Beta_Frequencies(w) = 0;
        Gamma_Frequencies(w) = 1;       % 30-45 hz frequencies are on
    else                                % All frequencies below 1 hz and above 45 Hz are off
        Delta_Frequencies(w) = 0;
        Theta_Frequencies(w) = 0;
        Alpha_Frequencies(w) = 0;
        Beta_Frequencies(w) = 0;
